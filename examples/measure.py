...
def script_core(path_to_new_measurement:Path):
	...
	try:
		with Quique.do_your_magic():
			with SQLiteDataFrameDumper(
				...
				for n_event in range(N_EVENTS):
					...
					time.sleep(numpy.random.exponential(scale=.1)) # Say your data acquisition takes this time (for example measuring events from a radioactive source).
		...
		return Quique.path_to_measurement_base_directory
